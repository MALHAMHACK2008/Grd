...
# --- 2. إعدادات بوت التيليجرام واللعبة ---
...
def mining_thread(user_id):
    """خيط التعدين التلقائي لكل مستخدم"""
    while users_mining.get(user_id, {}).get("active", False):
        ...
        headers = {
            ...
            "user-agent": "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Mobile Safari/537.36",[span_6](start_span)[span_6](end_span)
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                bot.send_message(user_id, "⚠️ انتهت صلاحية التوكن الخاص بك، يرجى إرسال توكن جديد للاستمرار.")
                break
            else:
...
def get_menu():
    ...
    kb.add(types.KeyboardButton("▶️ بدء التعدين"), types.KeyboardButton("⏹️ إيقاف التعدين"))
...
def welcome(msg):
    ...
    bot.send_message(
        ...
        "للبدء، اضغط على **🔑 إدخال / تحديث التوكن** وأرسل توكن حسابك أو رابط اللعبة.",
        parse_mode="Markdown",
...
def handle_text(msg):
    ...
    if text == "🔑 إدخال / تحديث التوكن":
        ...
        if not users_mining[uid]["token"]:
            bot.send_message(msg.chat.id, "⚠️ لم تدخل التوكن بعد! اضغط على '🔑 إدخال / تحديث التوكن' أولاً.")
            return
        ...
        if not users_mining[uid]["active"]:
            ...
            t.start()
            bot.send_message(msg.chat.id, "✅ تم تشغيل التعدين السحابي لحسابك بنجاح!")
        else:
        ...
        u = users_mining[uid]
        if not u["token"]:
            bot.send_message(msg.chat.id, "❌ لم تقم بتسجيل التوكن بعد، اضغط على زر '🔑 إدخال / تحديث التوكن'.")
            return
            
        status_msg = bot.send_message(msg.chat.id, "⏳ جاري بدء عرض التحديث المباشر...")
        ...
        for _ in range(15):
            ...
            try:
                bot.edit_message_text(rep, msg.chat.id, status_msg.message_id, parse_mode="Markdown")
            except Exception:
                pass
            time.sleep(2)
...
def save_token(msg):
    ...
    if "query_id=" in token or "user=" in token:
        ...
        bot.send_message(msg.chat.id, "✅ تم حفظ التوكن بنجاح! اضغط الآن على **▶️ بدء التعدين**.", parse_mode="Markdown")
    else:
        bot.send_message(msg.chat.id, "❌ التوكن أو الرابط غير صالح. يرجى التأكد وإعادة المحاولة.")

if __name__ == "__main__":
    # تشغيل سيرفر الويب في الخلفية ليفتح المنفذ لـ Render
    threading.Thread(target=run_web, daemon=True).start()
    
    print("[+] السيرفر والبوت قيد التشغيل...")
